...
class MeLUModel(torch.nn.Module):
    def __init__(self, config):
        ...
        self.interaction = nn.Sequential(
            self.fc1, nn.ReLU(), self.fc2, nn.ReLU(), self.linear_out
        )
        # new for gender fair
        ...
        if self.adv:
            ...
            if self.adv2:
                if config.out2 == 0:
                    ...
                    if config.loss == 0:
                        ...
                        self.fc_user_out_2 = Linear(
                            self.fc2_in_dim // 2, config.num_gender
                        )
                    elif config.loss == 1:
                    ...
                    if self.item_adv:
                        self.fc_item_1 = Linear(
                            self.fc2_in_dim // 2, self.fc2_in_dim // 2
                        )
                elif config.out2 == 2:
                    if config.loss == 0:
            ...
            if self.adv2:
                self.user_part_2 = nn.Sequential(
                    self.fc_user_2, nn.ReLU(), self.fc_user_out_2
                )
            ...
            if config.inner_fc == 1:
                ...
                self.local_max_part = nn.ModuleList(
                    [self.fc_user, self.fc_item, self.interaction]
                )
            if config.outer == 0:
                if config.train_adv:
                    self.define_task_adv()
                    self.global_part = nn.ModuleList(
                        [
                            self.item_emb,
                            self.user_emb,
                            self.fc_user,
                ...
                if config.train_adv:
                    ...
                    self.global_part = nn.ModuleList(
                        [self.item_emb, self.user_emb, self.fc_item, self.interaction]
                    )
    ...
    def define_task_adv(self):
        ...
        if self.dual:
            self.task_adv.append(
                nn.Parameter(torch.tensor([self.adv, self.adv], requires_grad=True))
            )
        else:
            self.task_adv.append(
                nn.Parameter(torch.tensor(self.adv, requires_grad=True))
            )
        ...
        if self.adv:
            ...
            if self.all_adv:
                ...
                if self.out == 0:
                    ...
                    if self.loss == 0:
                        ...
                        y_one_hot = F.one_hot(
                            torch.squeeze(y.long() - 1), num_classes=5
                        )
            ...
            if self.adv2:
                if self.out2 == 0:
                    ...
                    x_user = torch.mean(x_user, 0, True)
                    adv_loss += (
                        self.adv2 / self.adv * self.adv_loss(x_user, gender_idx[0:1])
                    )
                elif self.out2 == 2:
                    if self.loss == 0:
                        x_in = torch.cat([x, y], 1)
                    ...
                    x_user = F.relu(self.fc_user_2(x_in))
                    if self.item_adv:
                        x_user = x_user.clone() + F.relu(self.fc_item_1(item_emb))
                    x_user = self.fc_user_out_2(x_user)
                    x_user = torch.mean(x_user, 0, True)
                    adv_loss += (
                        self.adv2 / self.adv * self.adv_loss(x_user, gender_idx[0:1])
