...
def parse_args():
    parser = argparse.ArgumentParser(
        [],
        description="Fast Context Adaptation via Meta-Learning (CAVIA),"
    ...
    parser.add_argument(
        "--task", type=str, default="multi", help="problem setting: sine or celeba"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--data_root", type=str, default="./movielens/ml-1m", help="path to data root"
    )
    parser.add_argument(
        "--num_workers", type=int, default=4, help="num of workers to use"
    )
    parser.add_argument(
        "--test", action="store_true", default=False, help="num of workers to use"
    )
    parser.add_argument("--topk", type=int, default=3, help="num of workers to use")

    parser.add_argument(
        "--embedding_dim", type=int, default=32, help="num of workers to use"
    )
    parser.add_argument(
        "--first_fc_hidden_dim", type=int, default=64, help="num of workers to use"
    )
    parser.add_argument(
        "--second_fc_hidden_dim", type=int, default=64, help="num of workers to use"
    )
    parser.add_argument(
        "--num_epoch", type=int, default=60, help="num of workers to use"
    )
    parser.add_argument(
        "--num_genre", type=int, default=19, help="num of workers to use"
    )
    parser.add_argument(
        "--num_year", type=int, default=70, help="num of workers to use"
    )

    parser.add_argument(
        "--num_gender", type=int, default=2, help="num of workers to use"
    )
    parser.add_argument("--num_age", type=int, default=2, help="num of workers to use")
    parser.add_argument(
        "--num_occupation", type=int, default=21, help="num of workers to use"
    )
    parser.add_argument(
        "--num_zipcode", type=int, default=522, help="num of workers to use"
    )
    ...
    parser.add_argument(
        "--fair", action="store_true", default=False, help="train fair classifier"
    )
    parser.add_argument(
        "--fair_lr", type=float, default=0.001, help="train fair classifier"
    )
    parser.add_argument(
        "--fm", action="store_true", default=False, help="get fairness metric"
    )
    parser.add_argument(
        "--remove_gender",
        action="store_true",
        default=False,
        help="train fair classifier",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="dual trainable adv coefficient",
    )
    parser.add_argument(
        "--train_adv",
        action="store_true",
        default=False,
        help="trainable adv coefficient",
    )
    parser.add_argument(
        "--item_adv",
        action="store_true",
        default=False,
        help="item embedding input to adv, only 0|1",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="0: no group, 1: parity, 2: user perf, 3: value",
    )
    parser.add_argument(
        "--lam", type=float, default=0, help="0: no group, others: coeff"
    )
    parser.add_argument(
        "--con", action="store_true", default=False, help="counterfactual loss"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="0: update fc outer, 1: not update fc outer",
    )
    parser.add_argument(
        "--inner_fc",
        type=int,
        default=0,
        help="0: update fc inner, 1: not update fc inner",
    )
    parser.add_argument("--more", type=int, default=1, help="1: normal, 0: other")
    parser.add_argument(
        "--adv_loss_power", type=float, default=1, help="1: normal, other: other"
    )
    parser.add_argument(
        "--disable_inner_max",
        action="store_true",
        default=False,
        help="1: x inner max, 0: remain inner max",
    )
    parser.add_argument(
        "--disable_inner_adv",
        action="store_true",
        default=False,
        help="1: x inner adv, 0: remain inner adv",
    )
    parser.add_argument(
        "--schedule", action="store_true", default=False, help="1: schedule, 0: not"
    )
    parser.add_argument(
        "--all_adv", action="store_true", default=False, help="1: schedule, 0: not"
    )
    parser.add_argument(
        "--normalize",
        action="store_true",
        default=False,
        help="1: normalize for adv input, 0: not",
    )
...
def run_adv(args, num_workers=1, log_interval=100, verbose=True, save_path=None):
    ...
    if args.outer == 1:
        meta_grad_global_fix_init = [
            0 for _ in range(len(model.global_fix_part.state_dict()))
        ]

    # loading data
    dataloader_train = DataLoader(
        Metamovie(args), batch_size=1, num_workers=args.num_workers
    )
        ...
        for step, batch in enumerate(dataloader_train):
            ...
            for sth in gender_list:
                if sth == 0:
                    gender_w.append(
                        ...
                        / (
                            n_male_batch
                            + n_female_batch * n_male_users / n_female_users
                        )
                    )
                elif sth == 1:
                    gender_w.append(
                        (n_male_users / n_female_users)
                        / (
                            n_male_batch
                            + n_female_batch * n_male_users / n_female_users
                        )
            ...
            for i in range(args.tasks_per_metaupdate):
                ...
                if args.loss == 1:
                    loss_pre.append(
                        F.cross_entropy(
                            logits, torch.squeeze(y_qry[i].long()) - 1
                        ).item()
                    )
                else:
                ...
                for l in range(args.num_grad_steps_inner):
                    ...
                    if args.loss == 1:
                        if args.disable_inner_max:
                            loss_max = F.cross_entropy(
                                logits, torch.squeeze(y_spt[i].long()) - 1
                            )
                        else:
                            loss_max = (
                                F.cross_entropy(
                    ...
                    if not args.disable_inner_adv:
                        ...
                        grad = torch.autograd.grad(
                            loss, fast_parameters, create_graph=True
                        )
                        # print(grad)
                ...
                if args.loss == 1:
                    loss_q_global = (
                        F.cross_entropy(logits_q, torch.squeeze(y_qry[i].long()) - 1)
                        - adv * adv_loss**args.adv_loss_power / args.adv_loss_power
                    )
                else:
                    loss_q_global = (
                        F.mse_loss(logits_q, y_qry[i])
                        - adv * adv_loss**args.adv_loss_power / args.adv_loss_power
                    )
                if args.con:
                    # now use coeff 1
                    loss_q_global += 1 * F.mse_loss(output[2], logits_q)
                task_grad_test_global = torch.autograd.grad(
                    loss_q_global, model.global_part.parameters(), create_graph=True
                )
                for g in range(len(task_grad_test_global)):
                    if args.re_w:
                        meta_grad_global[g] += (
                            task_grad_test_global[g].detach() * gender_w[i]
                        )
                    else:
                        meta_grad_global[g] += task_grad_test_global[
                            g
                        ].detach() / float(args.tasks_per_metaupdate)
                if args.loss == 1:
                ...
                task_grad_test_local = torch.autograd.grad(
                    loss_q_local, model.local_part.parameters(), create_graph=True
                )  # , create_graph=True
                for g in range(len(task_grad_test_local)):
                    if args.re_w:
                        meta_grad_local[g] += (
                            task_grad_test_local[g].detach() * gender_w[i]
                        )
                    else:
                        meta_grad_local[g] += task_grad_test_local[g].detach() / float(
                            args.tasks_per_metaupdate
                        )
                if args.loss == 1:
                    loss_after.append(
                        F.cross_entropy(
                            logits_q, torch.squeeze(y_qry[i].long()) - 1
                        ).item()
                    )
                else:
            ...
            logger.train_conf.append(
                1.96 * np.std(loss_pre, ddof=0) / np.sqrt(len(loss_pre))
            )
            logger.valid_conf.append(
                1.96 * np.std(loss_after, ddof=0) / np.sqrt(len(loss_after))
            )
            ...
            print(
                "adv loss pre:{}, adv_loss_after:{}".format(
                    np.mean(adv_loss_pre), np.mean(adv_loss_after)
                )
            )
            logging.info(
                "adv loss pre:{}, adv_loss_after:{}".format(
...
def fairness_metrics(group_item_pred, group_item_rate):
    ...
    for item in shared_items:
        ...
        sign_de_male = np.mean(
            np.sign(np.array(male_pred[item]) - np.array(male_rate[item]))
        )
        sign_de_female = np.mean(
            np.sign(np.array(female_pred[item]) - np.array(female_rate[item]))
        )
    ...
    print(
        value_fairness,
        absolute_fairness,
        underestimation_fairness,
        overestimation_fairness,
        non_parity_fairness,
        user_value_fairness,
        user_abs_fairness,
    )
    logging.info(
        str(value_fairness)
        + " "
        + str(absolute_fairness)
        + " "
        + str(underestimation_fairness)
        + " "
        + str(overestimation_fairness)
        + " "
        + str(non_parity_fairness)
        + " "
        + str(user_value_fairness)
        + " "
        + str(user_abs_fairness)
    )
    male_preds, female_preds = [], []
    male_values, female_values = [], []
    male_abs, female_abs = [], []
    for item in shared_items:
        ...
        female_abs.extend(
            abs(np.array(female_pred[item]) - np.array(female_rate[item]))
        )
    ...
    print(
        "shared user fair",
        shared_non_parity_fairness,
        shared_user_value_fairness,
        shared_user_abs_fairness,
    )
    logging.info(
        "shared fair "
        + str(shared_non_parity_fairness)
        + " "
        + str(shared_user_value_fairness)
        + " "
        + str(shared_user_abs_fairness)
    )
...
def evaluate_test_adv(args, model, dataloader):
            ...
            for k in range(args.num_grad_steps_inner):
                ...
                if args.loss == 1:
                    if args.disable_inner_max:
                        ...
                        loss_max = (
                            F.cross_entropy(logits, torch.squeeze(y_spt[i].long()) - 1)
                            - adv
                            * adv_loss**args.adv_loss_power
                            / args.adv_loss_power
                        )
    ...
    print(
        "MAE: {}, NDCG@{}: {}".format(np.mean(loss_all), args.topk, np.mean(ndcg_all))
    )
    ...
    logging.info(
        "user group fair:" + str(abs(np.mean(group_perf[0]) - np.mean(group_perf[1])))
    )
